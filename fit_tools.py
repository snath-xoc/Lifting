...
from pygam import LinearGAM, ExpectileGAM, s, te, l
...
def fit_mean_response(dir_out, models, scen_train, data, idx_l, wgt_l, linreg  = False, weighted = False, gam_arch = 'spline'):
    ...
    for model in models:
        ...
        for scen in scen_train:
            ...
            Tanglob_temp = data[model].sel({'scenario':scen,'run':1})['globmean'].groupby('time.year').mean().values
            Tanglob_temp = Tanglob_temp.reshape(-1)[~np.isnan(Tanglob_temp.reshape(-1))]

            nr_ts = Tanglob_temp.shape[0]
            Tanglob_train.append(lowess(Tanglob_temp,np.arange(nr_ts),return_sorted=False,frac=50/nr_ts,it=0))
        ...
        if linreg:
            ...
            for i_mon in range(12):
                ...
                for gp in np.arange(idx_l.sum()):
                    ...
                    y_train = data[model].sel({'scenario':scen_train,'run':1, 'gp':gp})['y'].values.reshape(-1,12)[:,i_mon]
                    y_train = y_train[~np.isnan(y_train)]

                    coeff[0,i_mon,gp], coeff[1,i_mon,gp],_,_,_ = linregress(
                    Tanglob_train, y_train)

                    residuals[:,i_mon,gp] = comp_residuals(coeff[:,i_mon,gp], Tanglob_train, y_train)
            ...
            for i_mon in range(12):
                ...
                lifted.append(lifting.lift(X_lift,idx_l, i_mon, defined_regions=regionmask.defined_regions.ar6.land))
            ...
            est = Parallel(n_jobs=10)(delayed(fit_monthly)(gam_arch, X_train.T,
                                                           np.hstack((\
                                                                      [lifted[i_mon].C[region][\
                                                                                        lifted[i_mon].max_iter[region]\
                                                                                       ][:,0].reshape(-1,1)\
                                                                for i_mon in range(12)])), 
                                                                     weighted = weighted)\
            ...
            res_temp = Parallel(n_jobs=10)(delayed(comp_residuals_gam)(est[i_reg], X_train.T,
                                                                       np.hstack((\
                                                                                  [lifted[i_mon].C[region][\
                                                                                        lifted[i_mon].max_iter[region]\
                                                                                       ][:,0].reshape(-1,1)\
                                                                for i_mon in range(12)])), monthly = True)\
                                              for i_reg,region in enumerate(lifted[0].pairs.keys())
                                          )
                                     

            residuals = np.hstack([res_temp[i].reshape(-1,1) for i in range(len(res_temp))])
            ...
            if gam_arch=='spline': ## for keeping conventions
                ...
                if weighted:
                    joblib.dump(est, dir_out+model+'/mean_response/gam_reglift_wgt.pkl')
                    joblib.dump(residuals, dir_out+model+'/mean_response/gam_residuals_reglift_wgt.pkl')
                else:
                    joblib.dump(est, dir_out+model+'/mean_response/gam_reglift.pkl')
                    joblib.dump(residuals, dir_out+model+'/mean_response/gam_residuals_reglift.pkl')
                    joblib.dump(residuals_gp, dir_out+model+'/mean_response/residuals_lifted.pkl')
            else:
                
                if weighted:
                    joblib.dump(est, dir_out+model+'/mean_response/gam_'+gam_arch+'reglift_wgt.pkl')
                    joblib.dump(residuals, dir_out+model+'/mean_response/gam_'+gam_arch+'_residuals_reglift_wgt.pkl')
                    joblib.dump(residuals_gp, dir_out+model+'/mean_response/'+gam_arch+'_residuals_lifted_wgt.pkl')
                else:
                    joblib.dump(est, dir_out+model+'/mean_response/gam_'+gam_arch+'_reglift.pkl')
                    joblib.dump(residuals, dir_out+model+'/mean_response/gam_'+gam_arch+'_residuals_reglift.pkl')
                    joblib.dump(residuals_gp, dir_out+model+'/mean_response/'+gam_arch+'_residuals_lifted.pkl')
...
def comp_residuals_lifted(est, lifted, X, y):
    ...
    grounded_grid = Parallel(n_jobs=10)(delayed(\
                                                lifted[i_mon].ground_over_regions)(\
                                                                                   [est[i_reg][i_mon].predict(X)\
                                                                                    for i_reg, region in\
                                                                                    enumerate(lifted[0].pairs.keys())], 
                                                                                   n_samp = 1, parallel = False
                                                                                  )
                                        for i_mon in range(12)
                                       )
