...
class lift(object):
    ...
    def __init__(self, X, idx_l, i_mon, compression_factor=0.05, defined_regions=regionmask.defined_regions.giorgi,
                grid_resolution=2.5):
    ...
    def get_region_with_buffer(self, reg):
        ...
        idx_box = np.logical_and(np.isin(self.lons,lons_box),np.isin(self.lats,lats_box))
        land_sea_mask_box = self.idx_l[idx_box].reshape(mask_box.shape[0],mask_box.shape[1])
        ...
        return lats_box, lons_box, land_sea_mask_box, buffer_full_grid[self.idx_l], \
    sea_mask_box, land_mask_box, buffer_and_land
    ...
    def _group_geo_neighbours(self, reg, it):
        ...
        for i_lat in lats_reg:
            ...
            idx_lat = np.atleast_1d(idx_initial_x[np.squeeze(np.argwhere(lats==i_lat))])
            ...
            if not self._check_for_odd_cluster(idx_lat):
                ...
                if len(idx_lat)>3:
                    ...
                    if i_lat != lats_reg[-1]:
                        ...
                        lats[np.squeeze(np.argwhere(lats==i_lat))] = lats_reg[i_count+1]
                        
                    else:
                        ## unless we are at last latitude in which case merge to pair before
                        if pair_types[-1]=='odd':
                            pair_types[-1]='even'
                            pairs.append(np.array([pairs[-1][-1],idx_lat[0]]).reshape(2,-1))
                            pair_types.append('even')
                            pairs[-2] = pairs[-2][:-1].reshape(2,-1)
                        else:
                            ## either cut off last group into odd pair or if only one group make odd
                            if pairs[-1].shape[1]!=1:
                                pairs.append(np.array([pairs[-1][0,-1],pairs[-1][1,-1],idx_lat[0]]))
                                pair_types.append('odd')
                                pairs[-2] = pairs[-2][:,:-1]
                            else:
                                pairs[-1]=np.array([pairs[-1][0,-1],pairs[-1][1,-1],idx_lat[0]]) 
                                pair_types[-1]='odd'
    ...
    def _get_full_group(self, reg, it, x0):
        ...
        for i_count in np.arange(it+1)[::-1]:
            ...
            for pair, pair_type in zip(self.pairs[reg][i_count], self.pair_types[reg][i_count]):
    ...
    def _predict(self, reg, it):
        ...
        _, _, _, buffer_full_grid_l, _, _, buffer_and_land  = self.get_region_with_buffer(reg)
        ...
        for pair, pair_type in zip(self.pairs[reg][it], self.pair_types[reg][it]):
            ...
            if pair_type=='even':
                ...
                for x_val, y_val in zip(pair[0,:], pair[1,:]):
                    ...
                    c[:, members] = np.repeat(\
                                         (vals[:,x_val] + self.lamb[reg][it][y_val]*self.d[reg][it][:,y_val]).reshape(-1,1),
                                         len(members), axis=1,
                                        )
                ...
                c[:,members] = np.repeat(\
                                         (vals[:,pair[0]] + self.lamb[reg][it][pair[1]]*(\
                                                                             self.d[reg][it][:,pair[1]]+\
                                                                                         self.d[reg][it][:,pair[2]]
                                                                            )).reshape(-1,1),
                                         len(members), axis=1,
                                        )
    ...
    def summary_plot(self, reg, it, lat_pc, lon_pc, t=-1):
        ...
        lats, lons, _, buffer_full_grid_l, _, _, buffer_and_land  = self.get_region_with_buffer(reg)
        mask_reg_buffered = np.logical_or(buffer_full_grid_l,(self.regions==reg))
        ...
        axs.append(fig.add_subplot(1,3,1,projection=ccrs.Robinson(central_longitude=0)))
        axs.append(fig.add_subplot(1,3,2,projection=ccrs.Robinson(central_longitude=0)))
        axs.append(fig.add_subplot(1,3,3,projection=ccrs.Robinson(central_longitude=0)))
        ...
        for i_ax, ax in enumerate(axs):
            ...
            y_ma[self.idx_l] = j
            ax.coastlines()
            mesh = ax.pcolormesh(lon_pc, lat_pc, 
                                      y_ma, cmap=colors[i_ax], 
    ...
    def sample_from_nearest(self, reg, d_all, n_samp, d_prop=None):
        ...
        d_sampled = np.random.Generator(np.random.PCG64(seed)\
                                                        ).multivariate_normal(\
                                                                              mean, 
                                                                              cov, n_samp, 
                                                                            method='eigh')
    ...
    def sample_wavelet_coeff(self, y, n_samp, parallel=False, output_dsampled = False, d_prop = None, labels_ind = None,
                             compute_neighbourhood = True, **kwargs):
        ...
        '''  
        all_regs = self.C.keys() #np.unique(self.regions[~np.isnan(self.regions)]).astype(int)
        offset = 0
        ...
        if compute_neighbourhood:
            labels = np.hstack(([\
                np.vstack((\
                    [self.find_nearest(self.C[reg][-1][:,0], yval) for yval in y[reg+offset]]
                )) for reg in all_regs]
                               ))
            
            labels = np.vstack((\
                                [self.find_global_nearest(label) for label in labels]
            ))
        ...
        for reg in all_regs:
            ...
            if parallel:
                ...
                d_samp_temp = Parallel(n_jobs=10)(delayed(self.sample_from_nearest)(\
                                                                                    reg, d_all[reg][counter], 
                                                                                    n_samp, d_prop = d_prop_reg[counter],
                                                                                    **kwargs
                                                                                   )\
                ...
                for counter,_ in enumerate(labels):
                    ...
                    d_samples[reg][:,counter,:] = self.sample_from_nearest(reg, d_all[reg][counter], 
                                                                           n_samp, d_prop = d_prop_reg[counter], **kwargs)
        if output_dsampled:
            ...
            if pair_type=='even':
                ...
                for x_val, y_val in zip(pair[0,:], pair[1,:]):
                    ...
                    if it == 0:
                        ...
                        cinv[:, x_val] =  y[:,x_val]-self.lamb[reg][it][y_val]*d_sampled[:,y_val]
                        ...
                        cinv[:, members_x] =  np.repeat(\
                                                        (y[:,x_val]-self.lamb[reg][it][y_val]*d_sampled[:,y_val]).reshape(-1,1),
                                                  len(members_x), axis=1,
                                                 )

                        cinv[:, members_y] = np.repeat(\
                                                  (cinv[:, x_val] + d_sampled[:,y_val]).reshape(-1,1),
                                                  len(members_y), axis=1,
                                                 )
            else:
                ...
                if it == 0:
                    ...
                    cinv[:,members_x] = np.repeat(\
                                                  (y[:,pair[0]] - self.lamb[reg][it][pair[1]]*(\
                                                                                 d_sampled[:,pair[1]]\
                                                                                  +d_sampled[:,pair[2]]
                                                                                )).reshape(-1,1),
                                             len(members_x), axis=1,
                    ...
                    cinv[:,members_1] = np.repeat(\
                                             (cinv[:,pair[0]] + d_sampled[:,pair[1]]
                                               ).reshape(-1,1),
                                             len(members_1), axis=1,
                                            )

                    cinv[:,members_2] = np.repeat(\
                                             (cinv[:,pair[0]] + d_sampled[:,pair[2]]
                                               ).reshape(-1,1),
                                             len(members_2), axis=1,
                                            )
    ...
    def sample_and_update(self, reg, Y, Cinv, d_temp, sample):
        ...
        for it in np.arange(self.max_iter[reg]+1)[::-1]:
            ...
            if it==self.max_iter[reg]:
                ...
                Cinv[it].append(self.inverse_predict_and_update(reg, it, d_temp, Y.reshape(-1,1)))
                
            else:
                
                y_temp = Cinv[it+1][sample]
                
                Cinv[it].append(self.inverse_predict_and_update(reg, it, d_temp, y_temp))
    ...
    def ground(self, reg, Y, D_sampled, n_samp = 100, **kwargs):
        ...
        lats_box, lons_box, land_sea_mask_box, buffer_full_grid_l, sea_mask_box, land_mask_box, buffer_and_land\
        ...
        Cinv_temp = np.zeros([Cinv_final.shape[0],Cinv_final.shape[1],
                             buffer_and_land.shape[0], buffer_and_land.shape[1]])
        Cinv_temp[:,:,buffer_and_land] = Cinv_final
       
        return Cinv_temp[:,:,buffer_and_land]
            
    
    def ground_over_regions(self, Y_all, n_samp = 100, progress = False, parallel = True, 
                            d_prop = None, labels_ind = None, output_dsampled=False, compute_neighbourhood = True, **kwargs):
       
        ## Assume all regional time series are of equal length
        all_regs = self.C.keys()
        grounded_grid = np.full([n_samp, Y_all[0].shape[0], len(all_regs), self.idx_l.sum()], np.nan)
        
        start_time = time.time()
        D_sampled = self.sample_wavelet_coeff(Y_all, n_samp, parallel = False, output_dsampled = output_dsampled, 
                                              d_prop = d_prop, labels_ind = labels_ind, compute_neighbourhood = compute_neighbourhood)
        ...
        print('Time taken for sampling wavelets ----%s secs----'%(time.time()-start_time))
        ...
        if parallel:
            if progress: 
                Cinvs = Parallel(n_jobs=10)(delayed(self.ground) (int(reg), Y_all[int(reg)+offset], D_sampled
                                                                  , n_samp = n_samp, **kwargs)\
                ...
                print('Time taken for grounding ----%s secs----'%(time.time()-start_time))
            ...
            for reg in all_regs:
                ...
                grounded_grid[:,:,int(reg)+offset,mask_reg_buffered] =  Cinvs[int(reg)+offset]
            ...
            for reg in all_regs:
                ...
                Cinvs.append(self.ground(int(reg), Y_all[int(reg)+offset], D_sampled, n_samp = n_samp, **kwargs))
