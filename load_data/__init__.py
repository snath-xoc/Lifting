...
def get_cmip6_data(models, as_xarray=True, **kwargs):
    ...
    lon_pc, lat_pc, wgt, wgt_l, idx_l = extract_cmip6.get_land_mask(config['dir_meta_data'])
    
    data = {}
    
    for model in models:
        
        data[model] = extract_cmip6.load_data_single_mod(config['dir_cmip6_data'], model, idx_l, wgt, as_xarray=as_xarray,
                                                         **kwargs)
