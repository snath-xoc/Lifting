...
def get_land_mask(dir_data):
    ...
    frac_l = xr.open_mfdataset(dir_data+'interim_invariant_lsmask_regrid.nc', combine='by_coords',decode_times=False)
    ...
    idx_l=np.squeeze(frac_l.lsm.values)>0.43 # idex land #-> everything >0 I consider land
    ...
    ls["grid_raw"] = np.squeeze(mask_percentage(land_110, frac_l.lon, frac_l.lat).values)
...
def as_xr(y, run_nrs, T_ref, Tanglob_wgt, time_start="1850-01", time_stop="2101-01", usr_time_res="mon"):
    ...
    for scen in y.keys():
        ...
        if usr_time_res == "mon":
            ...
            times = np.arange(time_beg, time_end, np.timedelta64(1,'M'), dtype='datetime64')
                         ...
                         xr.Dataset(
                                    {
                                        ...
                                        "ref": ([ "gp"], T_ref),
                                        "globmean": (["scenario", "run", "time"], Tanglob_wgt_vals[None,:]),
...
def load_data_single_mod(dir_data, model, idx_l, wgt, Tref_start='1850-01-01',Tref_end='1900-01-01',usr_time_res="mon", var="tas", as_xarray=True, **kwargs):
    ...
    if var == 'hurs':
        ...
        dir_var = dir_data+var+'/'+var+'/'+usr_time_res+'/'+spatial_res+'/'
    
    run_names_list=sorted(glob.glob(dir_var+var+'_'+temp_res+'_'+model+'_ssp*_'+'r*i1p1f*'+'_'+spatial_res+'.nc'))
    run_names_list_historical = \
        sorted(glob.glob(dir_var+var+'_'+temp_res+'_'+model+'_historical_'+'r*i1p1f*'+'_'+spatial_res+'.nc'))
    
    for run_name in run_names_list:
        run_name_ssp = run_name
        data = xr.open_mfdataset(run_name_ssp).sel(time=slice('1850-01-01', '2101-01-01')).roll(lon=72)
        ...
        if scen not in list(y.keys()):
            ...
            y[scen][run] = data[var].values # still absolute values + still contains sea pixels
            Tan_wgt_globmean[scen][run] = np.multiply(y[scen][run],wgt[None,:,:]).sum(axis=(1,2))/wgt.sum() #area weighted but abs value
    ...
    for run_name in run_names_list_historical:
        ...
        data = xr.open_mfdataset(run_name_hist).sel(time=slice('1850-01-01', '2101-01-01')).roll(lon=72)
        

        data = data.assign_coords(lon= (((data.lon + 180) % 360) - 180))  # assign_coords so same labels as others
        run = int(run_name.split('/')[-1].split('_')[-2].split('r')[1].split('i')[0])                          
        
        y['historical'][run] = data[var].values # still absolute values + still contains sea pixels
        Tan_wgt_globmean['historical'][run] = np.multiply(y['historical'][run],wgt[None,:,:]).sum(axis=(1,2))/wgt.sum()
        run_nrs['historical'].append(run) 
            
            
        T_ref += data[var].sel(time=slice(Tref_start, Tref_end)).mean(dim='time').values*1.0/len(run_names_list_historical) # sum up all ref climates 
    ...
    if as_xarray:
        ...
        kwargs_as_xarray_dict = {k: kwargs.pop(k) for k in dict(kwargs) if k in kwargs_as_xarray}
        
        return as_xr(y, run_nrs, T_ref[idx_l], Tan_wgt_globmean, usr_time_res=usr_time_res, **kwargs_as_xarray_dict)
