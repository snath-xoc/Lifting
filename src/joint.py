...
class pair_ground(object):
    ...
    def calc_conditional_cov(self, d_sampled_raw, labels, **kwargs):
        ...
        for reg in d_sampled_raw.keys():
            ...
            for counter, label in enumerate(labels):
                ...
                sigma11 = (
                    skcovariance.EmpiricalCovariance().fit(d_ind[:, 1:]).precision_
                )
                sigma22 = np.cov(d_dep[:, 1:], rowvar=False)
        ...
        print(
            "Time taken to calculate conditional covariance matrix ----%s secs----"
    ...
    def ground_over_regions(
        self, Y, n_samp=100, progress=False, parallel=True, **kwargs
    ):

        grounded_grid_1, D_sampled_raw, labels = self.lift1.ground_over_regions(
            Y[0],
            n_samp=n_samp,
            progress=progress,
            parallel=parallel,
            output_dsampled=True,
        ...
        try:
            grounded_grid_2 = self.lift2.ground_over_regions(
                Y[1],
                n_samp=n_samp,
                progress=progress,
                parallel=parallel,
                d_prop=d_prop,
                labels_ind=np.empty(len(labels)),
                output_dsampled=False,
                compute_neighbourhood=False,
                **kwargs
            )
        except:
            print("Exited")
