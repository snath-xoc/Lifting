    ...
    if gam_arch == "spline":  ## for keeping conventions
        ...
        if weighted:
            ...
            residuals = joblib.load(
                dir_est + model + "/mean_response/gam_residuals_reglift_wgt.pkl"
            )
            residuals_gp = joblib.load(
                dir_est + model + "/mean_response/gam_residuals_lifted_wgt.pkl"
            )
        else:
            est = joblib.load(dir_est + model + "/mean_response/gam_reglift.pkl")
            residuals = joblib.load(
                dir_est + model + "/mean_response/gam_residuals_reglift.pkl"
            )
            residuals_gp = joblib.load(
                dir_est + model + "/mean_response/residuals_lifted.pkl"
            )
    else:

        if weighted:
            est = joblib.load(
                dir_est + model + "/mean_response/gam_" + gam_arch + "reglift_wgt.pkl"
            )
            residuals = joblib.load(
                dir_est
                + model
            ...
            est = joblib.load(
                dir_est + model + "/mean_response/gam_" + gam_arch + "_reglift.pkl"
            )
            residuals = joblib.load(
                dir_est
                + model
            ...
            residuals_gp = joblib.load(
                dir_est + model + "/mean_response/" + gam_arch + "_residuals_lifted.pkl"
            )
...
def get_regional_mr(residuals, i_mon, est, GMT, lifted, n_emu_mr):
    ...
    Y_all = [
        predict_mean_regional_response(
            est[region][i_mon], innov[:, :, int(region)], GMT
        )
...
def single_ground(Y_all, residuals_gp, i_mon, GMT, lifted, n_emu_mr, n_emu_lift):

    monthly_emulations = np.zeros(
        [n_emu_mr * n_emu_lift, GMT.shape[0], residuals_gp.shape[-1]]
    )
    ...
    for mr_emu in range(n_emu_mr):
        ...
        monthly_emulations[
            ...
            Y_mr_emu, n_samp=n_emu_lift, parallel=False
        )

    return [monthly_emulations]


def pair_ground(Y_all, residuals_gp, i_mon, GMT, lifted, n_emu_mr, n_emu_lift):
    ...
    monthly_emulations_1 = np.zeros(
        [n_emu_mr * n_emu_lift, GMT.shape[0], residuals_gp_1.shape[-1]]
    )
    monthly_emulations_2 = np.zeros(
        [n_emu_mr * n_emu_lift, GMT.shape[0], residuals_gp_1.shape[-1]]
    )
    ...
    for mr_emu in range(n_emu_mr):
        ...
        grounded_grid_1, grounded_grid_2 = pair_ground.ground_over_regions(
            [Y_mr_emu_1, Y_mr_emu_2], parallel=False, progress=False
        )
...
def loop_through_months(
    residuals, residuals_gp, i_mon, ests, GMT, lifted, n_emu_mr, n_emu_lift
):
    ...
    for i_est, est in enumerate(ests):
        ...
        Y_all_list.append(
            get_regional_mr(residuals[i_est], i_mon, est, GMT, lifted[i_est], n_emu_mr)
        )
    # print(len(Y_all_list),len(Y_all_list[0]), len(lifted))
    if pair:

        return pair_ground(
            Y_all_list, residuals_gp, i_mon, GMT, lifted, n_emu_mr, n_emu_lift
        )
    ...
    **kwargs
    ...
    if isinstance(dir_est, list) and isinstance(dir_out_emu, list):
        ...
        if len(dir_est) != len(dir_out_emu):
            ...
            print(
                "Estimator directory and output directory lists are not the same length"
            )
    ...
    if len(dir_est) != len(var):
        ...
        print(
            "Expected a separate directory for each variable, however len(dir_est)!=len(var)"
        )
    ...
    monthly_emulations = loop_through_months(
        all_residuals,
        all_residuals_gp,
        i_mon,
        all_ests,
    ...
    print(
        "--- %s seconds --- for %i emulations"
        % (time.time() - start_time, n_emu_mr * n_emu_lift)
    )
    ...
    for i_var, v in enumerate(var):
        ...
        nc_file = store_netcdf(
            emulations.reshape(
                n_emu_mr * n_emu_lift, -1, all_residuals_gp[i_var].shape[-1]
            ),
            time_beg=time_beg,
            time_end=time_end,
        )
        ...
        nc_file.to_netcdf(
            ...
            + dir_out_emu[i_var]
            + v
            + "_%iemus_" % (n_emu_mr * n_emu_lift)
            + model
            + "_"
            + scenario
            + "_%imon_g025.nc" % i_mon
        )
